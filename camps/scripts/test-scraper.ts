...
async function main() {
  ...
  if (args.length < 2) {
    ...
    console.error("Example: npx tsx scripts/test-scraper.ts .scraper-development/scraper-abc123.ts \"https://example.com\"");
    process.exit(1);
  }
  ...
  try {
    ...
    for (let i = 0; i < samples.length; i++) {
      ...
      if (s.dropOffHour !== undefined) console.log(`    ⏰ Time: ${s.dropOffHour}:${String(s.dropOffMinute || 0).padStart(2, "0")} - ${s.pickUpHour}:${String(s.pickUpMinute || 0).padStart(2, "0")}`);
      ...
      if (s.minGrade !== undefined || s.maxGrade !== undefined) console.log(`    🎓 Grades: ${s.minGrade ?? "?"}-${s.maxGrade ?? "?"}`);
