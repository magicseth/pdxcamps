...
export const runDataQualityChecks = internalAction({
  ...
  handler: async (
    ...
    const sources = await ctx.runQuery(internal.scraping.dataQualityChecks.getActiveSourcesForCheck);
    ...
    for (const source of sources) {
      ...
      if (source.dataQualityScore !== undefined && source.dataQualityScore < LOW_QUALITY_THRESHOLD) {
        issues.push({
          sourceId: source._id,
          sourceName: source.name,
      ...
      if (source.scraperHealth.lastSuccessAt && source.scraperHealth.lastSuccessAt < sevenDaysAgo) {
        const daysSinceSuccess = Math.floor((now - source.scraperHealth.lastSuccessAt) / (24 * 60 * 60 * 1000));
        issues.push({
          sourceId: source._id,
          sourceName: source.name,
      ...
      } else if (!source.scraperHealth.lastSuccessAt && source.scraperHealth.totalRuns > 0) {
    ...
    while (zeroPriceCursor !== undefined) {
      const page: {
        ...
        isDone: boolean;
        nextCursor?: number;
      } = await ctx.runQuery(internal.scraping.dataQualityChecks.findSourcesWithHighZeroPriceRatio, {
    ...
    for (const issue of issues) {
      const created = await ctx.runMutation(internal.scraping.dataQualityChecks.createAlertIfNotExists, {
        sourceId: issue.sourceId,
