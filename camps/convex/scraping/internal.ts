...
export const updateScraperHealth = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.success) {
      ...
      const needsRegeneration = newConsecutiveFailures >= 3 || currentHealth.needsRegeneration;
...
export const createRegenerationRequest = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const hasActiveRequest = existingBySource.some(
      (r) => r.status === 'pending' || r.status === 'in_progress' || r.status === 'testing',
    );
    ...
    const requestId = await ctx.db.insert('scraperDevelopmentRequests', {
      ...
      status: 'pending',
      scraperVersion: 0,
      notes: 'Auto-created: scraper flagged for regeneration due to consecutive failures or zero results',
    });
