 ...
 * ```
 */
async function executeDynamicScraper(code: string, config: ScraperConfig, log: ScraperLogger): Promise<ScrapeResult> {
  try {
    // Create a function from the code string
    // The function receives config, log, and fetch as parameters
    const AsyncFunction = Object.getPrototypeOf(async function () {}).constructor;

    const scraperFn = new AsyncFunction('config', 'log', 'fetch', 'cheerio', code);
    ...
    const result = await Promise.race([
      ...
      new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Scraper timeout')), timeoutMs)),
    ]);
        ...
        sessions: z.array(
          z.object({
            ...
            startDate: z
              .string()
              .optional()
              .describe('Start date in YYYY-MM-DD format. IMPORTANT: Convert any date format to YYYY-MM-DD.'),
            endDate: z
              .string()
              .optional()
              .describe('End date in YYYY-MM-DD format. IMPORTANT: Convert any date format to YYYY-MM-DD.'),
            ...
            dropOffHour: z
              .number()
              .optional()
              .describe('Drop-off hour in 24-hour format (0-23). Convert 9am to 9, 9pm to 21.'),
            ...
            availabilityRaw: z
              .string()
              .optional()
              .describe("Original availability text like '5 spots left' or 'Sold Out'"),
            ...
            sourceSessionId: z
              .string()
              .optional()
              .describe('Unique ID for this specific session if different from product'),
          }),
        ),
        organization: z
          .object({
...
export const scrapeSource = action({
  ...
  handler: async (
    ...
    const urlsToScrape: Array<{ url: string; label?: string }> = [{ url: source.url, label: 'Primary URL' }];
    ...
    for (const urlInfo of urlsToScrape) {
      try {
        ...
        const result = await ctx.runAction(api.scraping.scrapers.executor.executeStagehandScraper, {
          url: urlInfo.url,
          instruction,
          sourceId: args.sourceId,
          parsingNotes: source.parsingNotes,
