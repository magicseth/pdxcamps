...
export function validateSession(session: ScrapedSession): ValidationResult {
  ...
  if (
    session.startDate &&
    session.endDate &&
    isValidDateFormat(session.startDate) &&
    isValidDateFormat(session.endDate)
  ) {
  ...
  if (!session.location) {
    ...
    const isGeneric = genericLocations.some((gen) => session.location!.toLowerCase() === gen.toLowerCase());
    ...
    const cityPattern =
      /\b(Seattle|Portland|Redmond|Bellevue|Kirkland|Bothell|Issaquah|Sammamish|Mercer Island|Woodinville|Renton|Kent|Federal Way|Tacoma|Olympia|Beaverton|Lake Oswego|Tigard|Tualatin|Gresham|Hillsboro|Vancouver|Milwaukie|Clackamas|Oregon City|West Linn|Wilsonville)\b/i;
  ...
  const hasAgeRange = session.minAge !== undefined || session.maxAge !== undefined;
  const hasGradeRange = session.minGrade !== undefined || session.maxGrade !== undefined;
  ...
  const completenessScore = Math.round(((REQUIRED_FIELDS.length - missing.length) / REQUIRED_FIELDS.length) * 100);
 ...
 */
export function calculateSourceQuality(sessions: Array<{ completenessScore?: number }>): {
  score: number;
  ...
  const avgCompleteness = sessions.reduce((sum, s) => sum + (s.completenessScore ?? 0), 0) / sessions.length;
 ...
 */
export function determineSessionStatus(options: SessionStatusOptions | number): 'active' | 'draft' | 'pending_review' {
...
export function parseDateRange(
  ...
  const rangePattern1 = /(\w+)\s+(\d{1,2})\s*[-–]\s*(\d{1,2}),?\s*(\d{4})/i;
  ...
  const rangePattern2 = /(\d{1,2})\/(\d{1,2})\/(\d{4})\s*[-–]\s*(\d{1,2})\/(\d{1,2})\/(\d{4})/;
...
export function parseAgeRange(
  ...
  const gradePattern =
    /(?:grades?\s*)?(\d+|k|pre-?k)\s*(?:st|nd|rd|th)?\s*[-–]\s*(\d+|k)\s*(?:st|nd|rd|th)?(?:\s*grade)?/i;
