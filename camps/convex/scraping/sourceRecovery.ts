  ...
  > => {
    ...
    return inactiveSources
      .filter((source) => {
        ...
        return source.closedAt < sevenDaysAgo && /404/i.test(source.closureReason);
      })
      .map((source) => ({
        _id: source._id,
        name: source.name,
...
export const checkDisabledSources = internalAction({
  ...
  handler: async (ctx) => {
    ...
    const candidates = await ctx.runQuery(internal.scraping.sourceRecovery.getRecoveryCandidates, {});
    ...
    for (const source of candidates) {
      try {
        ...
        if (response.ok) {
          ...
          await ctx.runMutation(internal.scraping.sourceRecovery.reEnableSource, { sourceId: source._id });
    ...
    console.log(`[SourceRecovery] Checked ${results.length} sources, recovered ${recoveredCount}`);
