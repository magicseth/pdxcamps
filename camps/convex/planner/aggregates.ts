  ...
  handler: async (ctx, args) => {
    ...
    const sessions = await ctx.db
      ...
      .withIndex("by_city_and_status_and_start_date", (q) =>
        q
          .eq("cityId", args.cityId)
          .eq("status", "active")
          .lte("startDate", summerEnd)
      )
      .collect();

    // Filter: must overlap summer and have spots
    const available = sessions.filter(
      (s) => s.endDate >= summerStart && s.capacity > s.enrolledCount
    );
    ...
    for (const week of weeks) {
      const overlapping = available.filter(
        (s) => s.startDate <= week.endDate && s.endDate >= week.startDate
      );
    ...
    const existing = await ctx.db
      ...
      .withIndex("by_city_year", (q) =>
        q.eq("cityId", args.cityId).eq("year", args.year)
      )
      .unique();
...
export const getWeeklyAvailability = query({
  ...
  handler: async (ctx, args) => {
    ...
    const doc = await ctx.db
      .query("weeklyAvailability")
      .withIndex("by_city_year", (q) =>
        q.eq("cityId", family.primaryCityId).eq("year", args.year)
      )
      .unique();
