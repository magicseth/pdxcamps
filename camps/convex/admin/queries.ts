  ...
  handler: async (ctx, args) => {
    ...
    if (args.cityId) {
      ...
      const orgIdsInCity = new Set(
        organizations.filter((org) => org.cityIds.includes(args.cityId!)).map((org) => org._id),
      );
      sources = sources.filter((source) => source.organizationId && orgIdsInCity.has(source.organizationId));
    }
    ...
    const scrapeSuccessRate =
      activeSources > 0 ? Math.round(((activeSources - sourcesWithErrors) / activeSources) * 100) : 0;
    const dataSuccessRate = activeSources > 0 ? Math.round((sourcesWithSessions / activeSources) * 100) : 0;
  ...
  handler: async (ctx, args) => {
    ...
    const sourcesWithStats = await Promise.all(
      sources.map(async (source) => {
        ...
        const pendingForSource = pendingSessions.filter((p) => p.sourceId === source._id).length;
    ...
    const totalSources = sources.length;
    const activeSources = sources.filter((s) => s.isActive).length;
    const totalSessions = sourcesWithStats.reduce((sum, s) => sum + s.totalSessions, 0);
    const totalActiveSessions = sourcesWithStats.reduce((sum, s) => sum + s.activeSessions, 0);
    const sourcesWithSessions = sourcesWithStats.filter((s) => s.hasData).length;
    const sourcesWithoutSessions = sourcesWithStats.filter((s) => s.isActive && !s.hasData).length;
    const sourcesWithErrors = sourcesWithStats.filter((s) => s.health.consecutiveFailures > 0).length;

    // Quality breakdown
    const highQualitySources = sourcesWithStats.filter((s) => s.qualityTier === 'high').length;
    const mediumQualitySources = sourcesWithStats.filter((s) => s.qualityTier === 'medium').length;
    const lowQualitySources = sourcesWithStats.filter((s) => s.qualityTier === 'low').length;
  ...
  handler: async (ctx) => {
    ...
    const locationsNeedingFixes = locations.filter((loc) => {
      ...
      const hasPlaceholderStreet =
        !loc.address.street || loc.address.street === 'TBD' || loc.address.street.trim() === '';
    ...
    const orgMap = new Map(orgs.filter((o): o is Doc<'organizations'> => o !== null).map((o) => [o._id, o]));
