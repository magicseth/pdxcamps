      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Debug why camps aren&apos;t showing for a market
          </p>
        </div>
        <select
                  ...
                  <div className="flex items-center gap-6 text-sm">
                    <div className="text-center">
                      ...
                      <div className="font-semibold text-slate-900 dark:text-white">
                        {city.stats.organizations}
                      </div>
                    </div>
                    <div className="text-center">
                      <div className="text-slate-500">Sources</div>
                      <div className={`font-semibold ${city.stats.failingSources > 0 ? 'text-red-600' : 'text-slate-900 dark:text-white'}`}>
                    ...
                    <div className="text-center">
                      ...
                      <div className={`font-semibold ${city.stats.activeSessions === 0 ? 'text-red-600' : 'text-green-600'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4">
          ...
          <div className="space-y-2">
            <StatRow
              label="Healthy"
              count={pipeline.sources.healthy}
              total={pipeline.sources.total}
              color="green"
            />
            <StatRow
              label="Failing"
              count={pipeline.sources.failing}
              total={pipeline.sources.total}
              color="red"
            />
            <StatRow
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4">
          ...
          <div className="space-y-2">
            <StatRow
              label="Completed"
              count={recentJobs.byStatus.completed}
              total={recentJobs.total}
              color="green"
            />
            <StatRow
              label="Failed"
              count={recentJobs.byStatus.failed}
              total={recentJobs.total}
              color="red"
            />
            <StatRow
              label="Running"
              count={recentJobs.byStatus.running}
              total={recentJobs.total}
              color="blue"
            />
            <StatRow
              label="Pending"
              count={recentJobs.byStatus.pending}
              total={recentJobs.total}
              color="yellow"
            />
          </div>
              ...
              <div className="flex items-center gap-4 text-sm">
                <span className="text-slate-500">
                  {source.lastScrapedAt
                    ? `Last: ${formatTimestamp(source.lastScrapedAt)}`
                    : 'Never scraped'}
                </span>
                ...
                <span className="text-green-600 dark:text-green-400">
                  {source.activeSessionCount} active
                </span>
              </div>
            </Link>
          ))}
