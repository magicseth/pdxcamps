...
export default function ScraperDevPage() {
  return (
    <>
...
function ScraperDevContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.history.replaceState({}, '', '/admin/scraper-dev');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const needsFeedbackCount = requests.filter(r => r.status === 'needs_feedback').length;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Pending" value={pendingCount} variant="default" />
        <StatCard label="In Progress" value={inProgressCount} variant="info" />
        <StatCard label="Needs Feedback" value={needsFeedbackCount} variant="warning" />
...
function RequestRow({ request }: { request: any }) {
  ...
  const resetRequest = useMutation(api.scraping.development.resetToPending);
  ...
  const handleReset = async () => {
    await resetRequest({ requestId: request._id });
  };
