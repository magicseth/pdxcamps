# BOOTSTRAP PLAN: $500 Budget, Cash Flow Positive

## The Math

$500 in paid ads at $5 CPA = 100 users
100 users at 5% conversion = 5 paying customers
5 customers × $29 = $145

**That's a loss.** Paid ads don't work at this budget.

## New Strategy: Sell First, Scale With Revenue

We need to charge enough per customer that we're profitable on day one, then reinvest revenue into growth.

---

## WHAT PARENTS WILL PAY FOR RIGHT NOW

Parents are stressed. Camps are filling up. They'll pay for:

| Pain Point                          | Willingness to Pay |
| ----------------------------------- | ------------------ |
| "I don't have time to research"     | HIGH               |
| "I'm afraid I'll miss registration" | HIGH               |
| "I don't know what's good"          | MEDIUM             |
| "I can't compare prices easily"     | LOW                |

**Key insight:** The tool saves time, but some parents will pay MORE for someone to just do it for them.

---

## REVENUE MODEL: THREE TIERS

### Tier 1: Free Tool (Lead Generation)

- Browse all camps
- Basic search/filter
- View up to 2 weeks in planner
- **Purpose:** Get users in, show value, upsell

### Tier 2: Summer Pass - $39

- Unlimited planner access
- All children
- Registration deadline alerts
- "What's filling up" weekly email
- Exportable calendar
- **Target:** Self-service parents who want the tool

### Tier 3: Camp Concierge - $149

- Everything in Summer Pass
- **30-minute planning call** with a real person
- We build your summer plan for you
- Personalized camp recommendations
- "Done for you" peace of mind
- **Target:** Busy parents who want someone else to handle it

---

## WHY CONCIERGE WORKS

1. **High margin** - Your time costs ~$50/hour, you charge $149 for 30-60 min of work
2. **No product changes needed** - Just you + the tool you already built
3. **Deep user insight** - Learn exactly what parents struggle with
4. **Word of mouth** - Delighted customers tell friends
5. **Testimonials** - "PDX Camps planned my whole summer in one call"

**Math:**

- 10 concierge customers × $149 = $1,490
- Minus ~5 hours of your time
- Net: $1,400+ to reinvest in growth

---

## WEEK 1: VALIDATE AND SELL

### TASK: Add pricing page to site TODAY

**Deliverable:** Simple pricing page with three tiers

```
FREE               SUMMER PASS         CAMP CONCIERGE
$0                 $39                 $149

Browse camps       Everything free     Everything in Pass
Basic search       + Unlimited         + 30-min planning call
2-week preview       planner           + We build your plan
                   + All children      + Expert recommendations
                   + Deadline alerts   + Done for you
                   + Weekly updates

[Get Started]      [Get Summer Pass]   [Book Concierge]
```

### TASK: Add paywall to planner

**Implementation:**

- Let users see 2 weeks free
- When they try to add week 3+: "Upgrade to Summer Pass to plan your full summer"
- Show value before asking for money

### TASK: Set up Stripe checkout

**Products to create:**

- Summer Pass: $39 one-time
- Camp Concierge: $149 one-time
- Use Stripe Checkout for fast implementation

### TASK: Create Calendly for concierge calls

**Setup:**

- 30-minute slots
- Link Stripe payment to Calendly (pay first, then book)
- Or: Calendly with Stripe integration

---

## WEEK 1: FREE MARKETING (NO ADS)

### TASK: Facebook group launch post

**This is your "launch" - make it count**

Post to Portland Moms (19K members) and other groups:

```
"Hey Portland parents! 👋

I'm a local dad who got tired of checking 20 different websites to plan
summer camps. So I built something.

PDX Camps shows ALL Portland camps in one place—OMSI, Oregon Zoo,
Portland Parks, Trackers, and 100+ more. You can plan your whole
summer week-by-week and see what's still available.

It's free to browse and search. There's a paid version ($39) if you
want the full planner with deadline alerts.

And if you're totally overwhelmed, I'm doing "Camp Concierge" calls
where I'll literally plan your whole summer with you in 30 minutes.
A few parents have done it and said it was a lifesaver.

Would love feedback from Portland parents—what would make this more
useful for you?

pdxcamps.com"
```

**Why this works:**

- Introduces free tool (value first)
- Mentions paid options naturally
- Asks for feedback (engagement)
- Personal story (authentic)

### TASK: Post the deadline spreadsheet

**Same as before, but with soft pitch:**

```
"Made a spreadsheet of Portland camp registration deadlines—sharing
in case it helps!

[Google Sheet link]

Also built pdxcamps.com to see all camps in one place. Free to browse,
$39 if you want the full planner. Happy to answer questions!"
```

### TASK: Direct outreach to stressed parents

**When you see posts like:**

- "Help! I haven't planned camps yet!"
- "What camps still have space??"
- "I'm so overwhelmed with summer planning"

**Response:**

```
"I feel you! I built pdxcamps.com exactly for this—shows what's still
available across all Portland camps. Free to browse.

If you're really overwhelmed, I also do 30-min planning calls where
I'll help you map out your whole summer. A few parents have said it
saved them hours. DM me if interested!"
```

### TASK: Reach out to 10 friends/acquaintances with kids

**Personal outreach converts better than ads**

Text/email:

```
"Hey! Quick favor—I built a summer camp planning tool for Portland
parents and I'm looking for early feedback.

Would you be willing to try it and tell me what sucks?
It's at pdxcamps.com

Also offering a "Camp Concierge" thing where I hop on a call and
help plan your whole summer. Would that be useful to you?
First few are discounted while I'm testing it out."
```

---

## WEEK 1: CONCIERGE SALES PUSH

### TASK: Create "Camp Concierge" landing page

**Dedicated page that sells the service:**

```
# Camp Concierge: Your Summer, Planned

**Overwhelmed by summer camp planning?**

Let us do it for you.

In a 30-minute call, we'll:
✓ Review your family's schedule and needs
✓ Recommend camps that fit your kids' interests
✓ Build your complete week-by-week summer plan
✓ Flag registration deadlines you can't miss
✓ Answer all your questions

**$149 - Summer Stress: Solved**

[Book Your Call]

---

"I was drowning in browser tabs. After one call, my whole summer
was planned. Worth every penny." — Sarah, mom of 2

---

**What's included:**
- 30-minute video or phone call
- Personalized camp recommendations
- Complete summer plan in PDX Camps
- Deadline reminder setup
- Email follow-up with notes

**Limited availability** - I personally do these calls and only
have 10 spots this month.

[Book Your Call - $149]
```

### TASK: Offer "founding member" discount

**First 10 concierge customers: $99 (normally $149)**
**Creates urgency, gets testimonials**

### TASK: Post concierge offer directly

**In FB groups when relevant:**

```
"If anyone's completely overwhelmed—I'm doing a limited number of
'Camp Concierge' calls where I'll plan your whole summer with you
in 30 minutes. $99 for the first 10 families (normally $149).

I know all the Portland camps and have a tool that shows what's
still available. DM me if interested!"
```

---

## WEEK 2: DOUBLE DOWN ON WHAT WORKS

### IF Concierge is selling:

- Raise price to $149, then $199
- Add more slots
- Ask every customer for referrals and testimonials
- Use testimonials in marketing

### IF Summer Pass is selling:

- Create urgency: "Price goes up March 1"
- Add more free-to-paid triggers in the product

### IF neither is selling:

- Survey users: "What would you pay for?"
- Test lower price points ($19 pass, $79 concierge)
- Test different positioning/copy

---

## $500 BUDGET ALLOCATION

| Item                 | Cost | Purpose                    |
| -------------------- | ---- | -------------------------- |
| Stripe fees          | ~3%  | Payment processing         |
| Calendly (free tier) | $0   | Booking                    |
| One FB boost         | $50  | Boost best-performing post |
| Reserve              | $450 | Only spend if ROI proven   |

**Rule:** Don't spend on ads until you've made $500 in revenue.

---

## REVENUE TARGETS

**Week 1:**

- 3 Concierge sales × $99 = $297
- 5 Summer Pass sales × $39 = $195
- **Total: $492**

**Week 2:**

- 5 Concierge sales × $149 = $745
- 10 Summer Pass sales × $39 = $390
- **Total: $1,135**

**End of February:**

- 15 Concierge × $149 avg = $2,235
- 30 Summer Pass × $39 = $1,170
- **Total: $3,405**

Now you have budget for March ads.

---

## CASH FLOW POSITIVE LOOP

```
Week 1: Free channels → First sales ($500)
    ↓
Week 2: Reinvest $200 in FB boost → More sales ($1,000)
    ↓
Week 3: Reinvest $500 in ads → More sales ($2,000)
    ↓
Week 4: Reinvest $1,000 in ads → Scale ($4,000+)
    ↓
March: Real ad budget funded by customers
```

---

## WHAT TO BUILD (MINIMAL)

### Day 1 (hours):

- [ ] Pricing page
- [ ] Stripe checkout for both products
- [ ] Basic paywall on planner (week 3+ requires upgrade)
- [ ] Calendly setup for concierge

### Day 2-3:

- [ ] Concierge landing page
- [ ] Email receipt/welcome for purchasers
- [ ] "Upgrade" prompts in product

### Skip for now:

- Fancy features
- Complex integrations
- Perfect design

**Ship ugly, charge money, iterate.**

---

## CONCIERGE CALL SCRIPT

For the calls you sell:

**Before call (5 min prep):**

- Look at their kids' ages
- Note any camps they've already viewed/saved

**Call structure (30 min):**

1. **Intro (2 min)**
   "Thanks for booking! Tell me about your kids and what you're hoping for this summer."

2. **Gather requirements (5 min)**
   - Ages of kids
   - Weeks needing coverage
   - Budget per week
   - Interests (STEM, art, sports, nature)
   - Location preferences
   - Schedule constraints (drop-off/pickup times)

3. **Show options (10 min)**
   - Share screen, use PDX Camps
   - Filter by their criteria
   - Show 3-4 options per week
   - Explain tradeoffs

4. **Build the plan (10 min)**
   - Add camps to their planner
   - Fill each week
   - Note backup options

5. **Wrap up (3 min)**
   - Review the plan
   - Highlight registration deadlines
   - "I'll send a follow-up email with everything"

**After call:**

- Email summary with plan link
- Registration deadlines listed
- Ask for testimonial/referral

---

## AGENT EXECUTION PRIORITY

**Track A: Product (Day 1-2)**

- Implement paywall
- Add Stripe checkout
- Create pricing page
- Set up Calendly

**Track B: Sales (Day 1 onward)**

- Post to FB groups
- DM stressed parents
- Personal outreach to network
- Follow up on leads

**Track C: Fulfill (As sales come in)**

- Do concierge calls
- Collect testimonials
- Ask for referrals

**Track D: Iterate (Week 2+)**

- Review what's selling
- Adjust pricing
- Improve conversion

---

## SUCCESS METRICS

| Metric          | Week 1 | Week 2 | End of Feb |
| --------------- | ------ | ------ | ---------- |
| Revenue         | $500   | $1,000 | $3,000     |
| Concierge sales | 3      | 5      | 15         |
| Pass sales      | 5      | 10     | 30         |
| FB group posts  | 5      | 10     | 20         |
| Testimonials    | 1      | 3      | 8          |

---

## THE MINDSET

You're not building a startup with VC money. You're building a **business**.

- Every dollar spent must return more dollars
- Talk to customers constantly
- Charge more than you think you can
- Revenue is the only metric that matters right now

**February goal:** $3,000 revenue, 0 external funding needed.

**March goal:** Use February revenue to run real ads. Scale.
